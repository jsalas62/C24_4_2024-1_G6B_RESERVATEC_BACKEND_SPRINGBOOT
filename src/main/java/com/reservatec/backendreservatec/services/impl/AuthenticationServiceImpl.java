...
public class AuthenticationServiceImpl implements AuthenticationService {
    ...
    public Optional<Usuario> getAuthenticatedUser(Authentication authentication, OAuth2AuthenticationToken token) {
        if (!isAuthenticated(authentication)) {
            return Optional.empty();
        }
        return getUserFromToken(token).map(this::convertToEntity);
    }
}
